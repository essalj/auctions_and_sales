...
#for test use
def set_user(user, pw):
    sys.argv=('',user,pw)
    print(sys.argv)

set_user('essalj@yahoo.dk','8559sumirPd')
...
sys.argv=('','essalj@yahoo.dk', '8559sumirPd')
username = bytes(sys.argv[1], 'UTF-8')
password = bytes(sys.argv[2], 'UTF-8')

login_url = "https://api.dba.dk/api/v2/oauth/accesstoken"
login_headers = {'dbaapikey': '2abb0a87-9e2f-4bdd-3d79-08d3e9335416',
                 'User-Agent': 'dba/5.4.3 iPhone 10.0.1 (iPhone6,2)'}
r = requests.post(login_url, data={'username': username,
                                   'password': password},
    headers= login_headers, verify=verify).json()
